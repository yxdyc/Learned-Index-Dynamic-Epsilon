...
class RadixSpline {
 public:
  ...
  double GetEstimatedPosition(const KeyType key) const {
    ...
    if (key >= max_key_) return max_position_ - 1;
  ...
  SearchBound GetSearchBound(const KeyType key) const {
    ...
    // `end` is exclusive.
    const size_t end = (estimate + max_error_ + 2 > max_position_) ? max_position_ : (estimate + max_error_ + 2);
  ...
  size_t GetSplineSegment(const KeyType key) const {
    ...
    const KeyType prefix = static_cast<unsigned long>(key - min_key_) >> num_shift_bits_;

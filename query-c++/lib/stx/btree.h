...
struct btree_default_set_traits
{
    ...
    template <typename _Key, typename _Data, int _page_size=32>
    struct btree_map_traits
    {
        ...
        //static const int    leafslots = BTREE_MAX( 8, _page_size / (sizeof(_Key) + sizeof(_Data)) );
        ...
        static const int leaf_filling_factor = 1;  //default is '2',  setting as '1' indicates the full-paged filling
        static const int inner_filling_factor = 1; //default is '2',  setting as '1' indicates the full-paged filling
    };
...
template <typename _Key, typename _Data,
          ...
          //typename _Traits = btree_default_map_traits<_Key, _Data>,
            typename _Traits = btree_map_traits<_Key, _Data>,
...
public:
    ...
    static const unsigned short minleafslots = (leafslotmax / traits::leaf_filling_factor);

    /// Computed B+ tree parameter: The minimum number of key slots used
    /// in an inner node. If fewer slots are used, the inner node will be
    /// merged or slots shifted from it's siblings.
    // static const unsigned short mininnerslots = (innerslotmax / 2);
    static const unsigned short mininnerslots = (innerslotmax / traits::inner_filling_factor);
...
private:
    ...
    /// Searches for the first key in the node n smaller or equal to key. Uses
    /// binary search with an optional linear self-verification. This is a
    /// template function, because the slotkey array is located at different
    /// places in leaf_node and inner_node.
    template <typename node_type>
    inline int find_lower_smallerthan(const node_type *n, const key_type& key) const
    {
        if ( 0 && sizeof(n->slotkey) > traits::binsearch_threshold )
        {
            if (n->slotuse == 0) return 0;
            ...
            while (lo < hi)
            {
                ...
                if (key_greaterequal(key, n->slotkey[mid])) {
                    hi = mid; // key >= mid
                }
                else {
                    lo = mid + 1; // key < mid
            ...
            BTREE_PRINT("btree::find_lower_lessthan: on " << n << " key " << key << " -> " << lo << " / " << hi);

            // verify result using simple linear search
            if (selfverify)
            {
                int i = 0;
                while (i < n->slotuse && key_greater(n->slotkey[i],key)) ++i;

                BTREE_PRINT("btree::find_lower_lessthan: testfind: " << i);
                BTREE_ASSERT(i == lo);
            }
        ...
        {
            int lo = 0;
            while (lo < n->slotuse && key_greater(n->slotkey[lo],key)) ++lo;
            return lo;
        }
    }
...
public:
    ...
    leaf_node *find_approx_leaf(const key_type &key)
    {
        node *n = m_root;
        if (!n) return nullptr;
    ...
    const_iterator find_exact_leaf(const key_type &key, const leaf_node *leaf) const {
        //if (!leaf) return end();
    ...
    /// Searches the B+ tree and returns a constant iterator to the
    /// first pair equal to or smaller than key, or end() if all keys
    /// are smaller.
    inline const_iterator lower_bound_smallerthan(const key_type& key) const
    {
        const_iterator seg_leaf_iter_found = lower_bound(key);
        if (seg_leaf_iter_found.key() == key)
            return seg_leaf_iter_found;
        const_iterator seg_leaf_iter = iterator();
        if (seg_leaf_iter_found.currslot == 0 and seg_leaf_iter_found.currnode->prevleaf){
            seg_leaf_iter.currnode = seg_leaf_iter_found.currnode->prevleaf;
            seg_leaf_iter.currslot = seg_leaf_iter.currnode->slotuse - 1;
        } else{
            seg_leaf_iter.currnode = seg_leaf_iter_found.currnode;
            if (seg_leaf_iter_found.currslot != 0){
                seg_leaf_iter.currslot =  seg_leaf_iter_found.currslot - 1; // the last key less than the query
        ...
        if (seg_leaf_iter_found.currslot != 0){
            seg_leaf_iter.currnode = seg_leaf_iter_found.currnode;
            seg_leaf_iter.currslot = seg_leaf_iter_found.currslot - 1; // the last key less than the query
        } else if (seg_leaf_iter_found.currnode->prevleaf){ // the last leaf node
            seg_leaf_iter.currnode = seg_leaf_iter_found.currnode->prevleaf;
            seg_leaf_iter.currslot = seg_leaf_iter.currnode->slotuse - 1;
        }
        */
        return seg_leaf_iter;
    }
